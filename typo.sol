...
contract Typo is IfUserIsInGroup {
  ...
  Queue[] public Queues;
  // mappings // имя группы => адрес => состоит/несостоит
  // events
  ...
  // добавить сотрудника
  function addWorker(string _name, string _role, address _wallet) isInOneOfGroups(grsw) public {
    Workers.push(Worker(_name, _role, _wallet));
    isInGroup[_role][_wallet] = true;
  }
  // удалить сотрудника
